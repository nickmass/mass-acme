...
#[derive(Fail, Debug)]
enum AcmeError {
    #[fail(display = "Route53 DNS update took longer than {} seconds to propigate and has timed out", _0)]
    Route53PropigateTimeout(u64),
    #[fail(display = "Route53 returned an invalid response while waiting for propigation: {}", _0)]
...
fn run() -> Result<(), Error> {
    ...
    let _ = std::fs::create_dir_all(&*config.certificate_directory).context("Could not create SSL directory")?;
    ...
    let account = {
        ...
        if let Some(account_key) = load_key(&account_key_path)
            .context("Could not open account key")? {
    ...
    for cert in &config.certs {
        ...
        let _ = std::fs::create_dir_all(cert_dir)
            .context(format!("Unable to create cert directory for: {}", cert.name))?;
        ...
        if let Some(csr) = load_csr(&csr_file)
            .context(format!("Unable to read csr for: {}", cert.name))? {
        ...
        if let Some(key) = load_key(&key_file)
            .context(format!("Unable to read key for: {}", cert.name))? {
...
impl DnsClient {
    ...
    fn wait_for_change(&self, change_id: String) -> Result<(), Error> {
        ...
        loop {
            ...
            if total_time < 0 { Err(AcmeError::Route53PropigateTimeout(self.config.aws_max_wait))? }
